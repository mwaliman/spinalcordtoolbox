...
#
# tested with Spinal Cord Toolbox (v4.0.0_beta2)

# Exit if user presses CTRL+C (Linux) or CMD+C (OSX)
trap "echo Caught Keyboard Interrupt within script. Exiting now.; exit" INT

# Fetch OS type to define open command (for opening QC in your Browser)
if uname -a | grep -i  darwin > /dev/null 2>&1; then open_cmd="open"; else open_cmd="xdg-open"; fi
...
# Check QC report (it will open your browser automatically).
$open_cmd "/Users/julien/qc_singleSubj/index.html"
...
# Tips: Here we use the PAM50 contrast t1, which is closer to the dwi contrast (although we are not using type=im in -param, so it will not make a difference here)
